...
export default class RemoteDevice extends EventEmitter {
	...
	handleMessage=message=>{
		...
		if (cof.get(COF.COF_FUNC)==COF.COF_FUNC_HEARTBEAT &&
			...
			if (newState!=this.state) {
				...
				//console.log("state change id="+this.nodeId+" state="+this.state);
			}
	...
	flush() {
		//console.log("to flush: "+this.dirtyEntries.length);

		return new Promise((resolve, reject)=>{
			this.writeSemaphore.critical(()=>{
				/*if (global.gc)
					global.gc();*/
				...
				function handleWritten(e) {
					...
					//console.log("unwritten len: "+unwritten.length);
