...
export default class RemoteEntry extends EventEmitter {
	constructor(index, subIndex) {
  	...
  	subscribe({interval, pdoChannel}={}) {
  		...
  		if (pdoChannel) {
  			let entry=this.device.entry(0x1A00+pdoChannel-1,1);
  			entry.setData([32,this.subIndex,(this.index&0xff),(this.index>>8)]);
  		}
	...
	async expeditedWrite() {
	    ...
	    let res,delay=500,attempt=0;
	    while (!res) {
	    	attempt++;
	    	if (attempt>4) {
	    		throw new Error("expedited write timed out");
	    	}

	        this.device.getBus().send(cof);
	        res=await awaitEvent(this.device.getBus(),"message",m=>{
				let cof=new COF(m);
        ...
        this.emit("written",this);
        //console.log("exp write complete!!");
	}
